document.addEventListener('DOMContentLoaded', function() {
    ...
    form.addEventListener('submit', async function(e) {
        ...
        try {
            ...
            mainPrediction.textContent = className.charAt(0).toUpperCase() + className.slice(1);
            confidence.textContent = (result.confidence * 100).toFixed(2);

            // Update probabilities
            if (Array.isArray(result.all_probabilities)) {
                // Sort probabilities in descending order
                const sortedProbs = [...result.all_probabilities].sort((a, b) => b.probability - a.probability);
